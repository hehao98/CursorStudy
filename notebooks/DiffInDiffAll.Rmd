...
# Create ratio metrics before log transformation
# Avoid division by zero by adding a small constant
panel_data$code_smells_per_line <- panel_data$code_smells / (panel_data$lines_added + 1)
panel_data$vulnerabilities_per_line <- panel_data$vulnerabilities / (panel_data$lines_added + 1)
panel_data$bugs_per_line <- panel_data$bugs / (panel_data$lines_added + 1)
...
# Log transform the new ratio metrics
panel_data$code_smells_per_line <- log(panel_data$code_smells_per_line + 1)
panel_data$vulnerabilities_per_line <- log(panel_data$vulnerabilities_per_line + 1)
panel_data$bugs_per_line <- log(panel_data$bugs_per_line + 1)
...
```{r dynamic_effects, fig.width=10.5, fig.height=5}
...
## Dynamic Treatment Effects - Velocity Outcomes Only

```{r velocity_dynamic_plot, fig.width=8, fig.height=4}
# Define velocity-related outcomes
velocity_vars <- c("commits", "lines_added")
velocity_names <- c("Commits", "Lines Added")

# Filter dynamic effects for velocity outcomes only
velocity_dynamic_effects <- dynamic_effects %>%
    filter(outcome %in% velocity_names)

# Create velocity-focused dynamic plot
velocity_dynamic_plot <- ggplot(velocity_dynamic_effects, aes(x = time, y = estimate, color = method)) +
    # Add significant points (filled)
    geom_point(data = filter(velocity_dynamic_effects, significant),
               position = position_dodge(width = 0.5), size = 2.5, shape = 19) +
    # Add non-significant points (hollow)
    geom_point(data = filter(velocity_dynamic_effects, !significant),
               position = position_dodge(width = 0.5), size = 2.5, shape = 21, fill = "transparent") +
    # Add error bars for significant effects (solid)
    geom_errorbar(data = filter(velocity_dynamic_effects, significant),
                  aes(ymin = conf.low, ymax = conf.high),
                  position = position_dodge(width = 0.5), width = 0.2, linetype = "solid") +
    # Add error bars for non-significant effects (dotted)
    geom_errorbar(data = filter(velocity_dynamic_effects, !significant),
                  aes(ymin = conf.low, ymax = conf.high),
                  position = position_dodge(width = 0.5), width = 0.2, linetype = "dotted") +
    geom_hline(yintercept = 0, linetype = "dashed", color = "red", size = 0.8) +
    geom_vline(xintercept = 0, linetype = "dashed", color = "blue", size = 0.8) +
    scale_color_manual(values = c(
        "Two Way Fixed Effects" = "#CC79A7",
        "Borusyak et al." = "#009E73",
        "Callaway and Sant'Anna" = "#0072B2"
    )) +
    facet_wrap(~outcome, scales = "free_y", ncol = 2) +
    scale_x_continuous(limits = c(-6, 6), breaks = -6:6) +
    labs(
        title = "Dynamic Treatment Effects: Velocity Outcomes",
        x = "Months Relative to Cursor Adoption",
        y = "Treatment Effect",
        color = "Estimator",
        caption = "Filled points: p < 0.05, Hollow points: p ≥ 0.05"
    ) +
    theme_minimal() +
    theme(
...
print(velocity_dynamic_plot)

ggsave("../plots/velocity_dynamic_effects.pdf",
  plot = velocity_dynamic_plot, width = 8, height = 4)
```

## Dynamic Treatment Effects - Quality Outcomes Only

```{r quality_dynamic_plot, fig.width=12, fig.height=8}
# Define quality-related outcomes
quality_vars <- c("code_smells", "vulnerabilities", "bugs",
                  "comment_lines_density", "duplicated_lines_density", "cognitive_complexity")
quality_names <- c("Code Smells", "Vulnerabilities", "Bugs",
                   "% Comment Lines", "% Duplicated Lines", "Cognitive Complexity")

# Filter dynamic effects for quality outcomes only
quality_dynamic_effects <- dynamic_effects %>%
    filter(outcome %in% quality_names)

# Create quality-focused dynamic plot
quality_dynamic_plot <- ggplot(quality_dynamic_effects, aes(x = time, y = estimate, color = method)) +
    # Add significant points (filled)
    geom_point(data = filter(quality_dynamic_effects, significant),
               position = position_dodge(width = 0.5), size = 2.5, shape = 19) +
    # Add non-significant points (hollow)
    geom_point(data = filter(quality_dynamic_effects, !significant),
               position = position_dodge(width = 0.5), size = 2.5, shape = 21, fill = "transparent") +
    # Add error bars for significant effects (solid)
    geom_errorbar(data = filter(quality_dynamic_effects, significant),
                  aes(ymin = conf.low, ymax = conf.high),
                  position = position_dodge(width = 0.5), width = 0.2, linetype = "solid") +
    # Add error bars for non-significant effects (dotted)
    geom_errorbar(data = filter(quality_dynamic_effects, !significant),
    ...
    scale_x_continuous(limits = c(-6, 6), breaks = -6:6) +
    labs(
        title = "Dynamic Treatment Effects: Quality Outcomes",
        x = "Months Relative to Cursor Adoption",
        y = "Treatment Effect",
        color = "Estimator",
        caption = "Filled points: p < 0.05, Hollow points: p ≥ 0.05"
...
print(quality_dynamic_plot)

ggsave("../plots/quality_dynamic_effects.pdf",
  plot = quality_dynamic_plot, width = 12, height = 8)
```

## Static Treatment Effects Table - Ratio Metrics Only

```{r ratio_static_table}
# Define ratio-related outcomes
ratio_vars <- c("code_smells_per_line", "vulnerabilities_per_line", "bugs_per_line")
ratio_names <- c("Code Smells per Line", "Vulnerabilities per Line", "Bugs per Line")

# Run all methods for ratio variables
ratio_callaway_results <- list()
ratio_borusyak_results <- list()
ratio_twfe_results <- list()

for (var in ratio_vars) {
    ratio_callaway_results[[var]] <- run_callaway(var, panel_data)
    ratio_borusyak_results[[var]] <- run_borusyak(var, panel_data)
    ratio_twfe_results[[var]] <- run_twfe(var, panel_data)
}

# Extract static effects for ratio variables
ratio_static_effects <- rbind(
    extract_static_effects(ratio_twfe_results, "TWFE", ratio_vars),
    extract_static_effects(ratio_borusyak_results, "Borusyak et al.", ratio_vars),
    extract_static_effects(ratio_callaway_results, "Callaway and Sant'Anna", ratio_vars)
)

# Create factor for proper ordering and labeling
ratio_static_effects$outcome <- factor(ratio_static_effects$outcome,
                                      levels = ratio_vars,
                                      labels = ratio_names)

# Create ratio-focused static table
ratio_static_table <- ratio_static_effects %>%
    mutate(
        # Calculate p-values (two-tailed test)
        z_stat = estimate / se,
        p_value = 2 * (1 - pnorm(abs(z_stat))),
...
# Reshape to wide format for better presentation
ratio_static_table_wide <- ratio_static_table %>%
    pivot_wider(
        id_cols = outcome,
        names_from = method,
        values_from = c(estimate_sig, se_formatted),
...
# Create the final table
kable(ratio_static_table_wide,
      col.names = c("Outcome",
                    "Estimate", "Std. Error",
                    "Estimate", "Std. Error",
                    "Estimate", "Std. Error"),
      caption = "Static Treatment Effects: Quality Ratio Metrics (Impact of Cursor Adoption)",
      align = c("l", rep("r", 6))) %>%
    kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                  full_width = FALSE,
                  font_size = 12) %>%
...
## Dynamic Treatment Effects - Ratio Metrics Only

```{r ratio_dynamic_plot, fig.width=12, fig.height=4}
# Run dynamic analysis for ratio variables
ratio_callaway_dynamic <- list()
ratio_borusyak_dynamic <- list()

for (var in ratio_vars) {
    ratio_callaway_dynamic[[var]] <- run_callaway(var, panel_data)

    ratio_borusyak_dynamic[[var]] <- did_imputation(
        data = panel_data,
        yname = var,
        gname = "event",
        tname = "time",
...
# Combine all dynamic effects for ratio variables
ratio_dynamic_effects <- rbind(
    extract_dynamic_effects(ratio_twfe_results, "Two Way Fixed Effects", ratio_vars),
    extract_dynamic_effects(ratio_borusyak_dynamic, "Borusyak et al.", ratio_vars),
    extract_dynamic_effects(ratio_callaway_dynamic, "Callaway and Sant'Anna", ratio_vars)
)

# Create factor for proper ordering and labeling
ratio_dynamic_effects$outcome <- factor(ratio_dynamic_effects$outcome,
                                        levels = ratio_vars,
                                        labels = ratio_names)

# Add significance indicators for visual styling
ratio_dynamic_effects <- ratio_dynamic_effects %>%
    mutate(
        # Calculate standard error from confidence intervals (approximate)
        se = (conf.high - conf.low) / (2 * 1.96),
        # Calculate z-statistic
...
# Create ratio-focused dynamic plot
ratio_dynamic_plot <- ggplot(ratio_dynamic_effects, aes(x = time, y = estimate, color = method)) +
    # Add significant points (filled)
    geom_point(data = filter(ratio_dynamic_effects, significant),
               position = position_dodge(width = 0.5), size = 2.5, shape = 19) +
    # Add non-significant points (hollow)
    geom_point(data = filter(ratio_dynamic_effects, !significant),
               position = position_dodge(width = 0.5), size = 2.5, shape = 21, fill = "transparent") +
    # Add error bars for significant effects (solid)
    geom_errorbar(data = filter(ratio_dynamic_effects, significant),
                  aes(ymin = conf.low, ymax = conf.high),
                  position = position_dodge(width = 0.5), width = 0.2, linetype = "solid") +
    # Add error bars for non-significant effects (dotted)
    geom_errorbar(data = filter(ratio_dynamic_effects, !significant),
    ...
    scale_x_continuous(limits = c(-6, 6), breaks = -6:6) +
    labs(
        title = "Dynamic Treatment Effects: Quality Ratio Metrics",
        x = "Months Relative to Cursor Adoption",
        y = "Treatment Effect",
        color = "Estimator",
        caption = "Filled points: p < 0.05, Hollow points: p ≥ 0.05"
...
print(ratio_dynamic_plot)

ggsave("../plots/ratio_dynamic_effects.pdf",
  plot = ratio_dynamic_plot, width = 12, height = 4)
```
