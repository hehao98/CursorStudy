...
summary_table <- static_effects %>%
    select(outcome, method, estimate, se) %>%
    pivot_wider(names_from = method, values_from = c(estimate, se)) %>%
    mutate(
        outcome = tools::toTitleCase(gsub("_", " ", outcome))
    )

knitr::kable(summary_table,
             digits = 3,
             caption = "Treatment Effects Summary by Method")
```
