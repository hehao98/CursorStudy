...
# Split outcomes: top panels vs last row (last 3 outcomes)
outcome_levels <- levels(dynamic_effects$outcome)
top_levels <- outcome_levels[seq_len(length(outcome_levels) - 3)]
last_levels <- tail(outcome_levels, 3)

dynamic_effects_top <- dynamic_effects %>% filter(outcome %in% top_levels)
dynamic_effects_last <- dynamic_effects %>% filter(outcome %in% last_levels)

# Create comparison plot (top panels)
dynamic_plot <- ggplot(dynamic_effects_top, aes(x = time, y = estimate, color = method)) +
    # Add significant points (filled)
    geom_point(data = filter(dynamic_effects_top, significant),
    ...
    geom_errorbar(data = filter(dynamic_effects_top, significant),
...
```{r dynamic_effects_lastrow, fig.width=7.5, fig.height=3}
# Standalone figure for the last row (last 3 outcomes)
dynamic_plot_last <- ggplot(dynamic_effects_last, aes(x = time, y = estimate, color = method)) +
    geom_point(data = filter(dynamic_effects_last, significant),
               position = position_dodge(width = 0.5), size = 2, shape = 19) +
    geom_point(data = filter(dynamic_effects_last, !significant),
               position = position_dodge(width = 0.5), size = 2, shape = 21, fill = "transparent") +
    geom_errorbar(data = filter(dynamic_effects_last, significant),
                  aes(ymin = conf.low, ymax = conf.high),
                  position = position_dodge(width = 0.5), width = 0, linetype = "solid") +
    geom_errorbar(data = filter(dynamic_effects_last, !significant),
                  aes(ymin = conf.low, ymax = conf.high),
...
print(dynamic_plot_last)

ggsave("../plots/dynamic_effects_lastrow.pdf",
  plot = dynamic_plot_last, width = 7.5, height = 3.0,
  device = cairo_pdf, family = "DejaVu Sans")
```
...
borusyak_pretrend_test <- function(bjs_dyn_df) {
    ...
    dyn_df$time_num <- suppressWarnings(as.numeric(dyn_df$term))
