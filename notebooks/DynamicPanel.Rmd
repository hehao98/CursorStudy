...
dynamic_data <- panel_data %>%
    ...
    filter(!is.na(code_smells) &
        !is.na(vulnerabilities) & !is.na(bugs) & !is.na(cognitive_complexity) &
...
pdyn <- pdyn %>%
    mutate(
        ...
        static_analysis_warnings = log(static_analysis_warnings + 1)
    )
...
cor_matrix <- pdyn %>%
    select(cognitive_complexity, static_analysis_warnings, commits, lines_added) %>%
...
dynamic_model_lines2 <- pgmm(
    formula = lines_added ~ lag(lines_added, 1) + lag(static_analysis_warnings, 1) +
        post_event +
        log(ncloc + 1) + log(age + 1) + log(contributors + 1) + log(stars + 1) +
        log(issues + 1) |
        lag(lines_added, 2),
...
```{r dynamic_model_static_analysis_warnings}
# Test: Velocity_{i,t} → TechnicalDebt_{it}
# Use lagged velocity as instrument for contemporaneous velocity

dynamic_model_static_analysis_warnings <- pgmm(
    formula = static_analysis_warnings ~ lag(static_analysis_warnings, 1) +
        lines_added +  # Contemporaneous effect (potentially endogenous)
        post_event +
        log(ncloc + 1) + log(age + 1) + log(contributors + 1) + log(stars + 1) +
        log(issues + 1) |
        lag(static_analysis_warnings, 2:3) + lag(lines_added, 2:3),
    data = pdyn,
    effect = "twoways",
    model = "twosteps",
    transformation = "d",
...
summary(dynamic_model_static_analysis_warnings, robust = TRUE)
```

## Alternative Models Using Commits as Productivity Measure

```{r dynamic_model_commits}
dynamic_model_commits <- pgmm(
    formula = commits ~ lag(commits, 1) + lag(cognitive_complexity, 1) +
        post_event +
        log(ncloc + 1) + log(age + 1) + log(contributors + 1) + log(stars + 1) +
        log(issues + 1) |
        lag(commits, 2),
...
summary(dynamic_model_commits, robust = TRUE)
```

```{r dynamic_model_commits2}
dynamic_model_commits2 <- pgmm(
    formula = commits ~ lag(commits, 1) + lag(static_analysis_warnings, 1) +
        post_event +
        log(ncloc + 1) + log(age + 1) + log(contributors + 1) + log(stars + 1) +
        log(issues + 1) |
        lag(commits, 2),
...
summary(dynamic_model_commits2, robust = TRUE)
```

```{r dynamic_model_complexity_commits}
dynamic_model_complexity_commits <- pgmm(
    formula = cognitive_complexity ~ lag(cognitive_complexity, 1) +
        commits +  # Contemporaneous effect (potentially endogenous)
        post_event +
        log(ncloc + 1) + log(age + 1) + log(contributors + 1) + log(stars + 1) +
        log(issues + 1) |
        lag(cognitive_complexity, 2:3) + lag(commits, 2:3),
    data = pdyn,
    effect = "twoways",
    model = "twosteps",
    transformation = "d",
...
summary(dynamic_model_complexity_commits, robust = TRUE)
```

```{r dynamic_model_static_analysis_warnings_commits}
dynamic_model_static_analysis_warnings_commits <- pgmm(
    formula = static_analysis_warnings ~ lag(static_analysis_warnings, 1) +
        commits +  # Contemporaneous effect (potentially endogenous)
        post_event +
        log(ncloc + 1) + log(age + 1) + log(contributors + 1) + log(stars + 1) +
        log(issues + 1) |
        lag(static_analysis_warnings, 2:3) + lag(commits, 2:3),
    data = pdyn,
    effect = "twoways",
    model = "twosteps",
    transformation = "d",
...
summary(dynamic_model_static_analysis_warnings_commits, robust = TRUE)
```
...
models_list <- list(
    ...
    "Lines Added\n(Static Analysis Warnings)" = dynamic_model_lines2,
    "Cognitive\nComplexity\n(Lines)" = dynamic_model_complexity,
    "Static Analysis Warnings\n(Lines)" = dynamic_model_static_analysis_warnings,
    "Commits\n(Complexity)" = dynamic_model_commits,
    "Commits\n(Static Analysis Warnings)" = dynamic_model_commits2,
    "Cognitive\nComplexity\n(Commits)" = dynamic_model_complexity_commits,
    "Static Analysis Warnings\n(Commits)" = dynamic_model_static_analysis_warnings_commits
)
...
diag4 <- get_diagnostics(dynamic_model_static_analysis_warnings)
diag5 <- get_diagnostics(dynamic_model_commits)
diag6 <- get_diagnostics(dynamic_model_commits2)
diag7 <- get_diagnostics(dynamic_model_complexity_commits)
diag8 <- get_diagnostics(dynamic_model_static_analysis_warnings_commits)
...
coef4 <- extract_coefs(dynamic_model_static_analysis_warnings)
coef5 <- extract_coefs(dynamic_model_commits)
coef6 <- extract_coefs(dynamic_model_commits2)
coef7 <- extract_coefs(dynamic_model_complexity_commits)
coef8 <- extract_coefs(dynamic_model_static_analysis_warnings_commits)
...
var_names <- c(
    ...
    "lag(commits, 1)" = "Commits (t-1)",
    "lines_added" = "Lines Added (t)",
    "commits" = "Commits (t)",
    "lag(cognitive_complexity, 1)" = "Cognitive Complexity (t-1)",
    "lag(static_analysis_warnings, 1)" = "Static Analysis Warnings (t-1)",
...
create_row <- function(var, label, c1, c2, c3, c4, c5, c6, c7, c8) {
    ...
    tibble::tribble(
        ...
        label, v1$est, v2$est, v3$est, v4$est, v5$est, v6$est, v7$est, v8$est,
        "", v1$se, v2$se, v3$se, v4$se, v5$se, v6$se, v7$se, v8$se
    )
}
...
table_rows <- bind_rows(
    lapply(names(var_names), function(v) {
        create_row(v, var_names[v], coef1, coef2, coef3, coef4, coef5, coef6, coef7, coef8)
    })
)
...
table_rows <- bind_rows(
    ...
    tibble::tribble(
        ~Variable, ~M1, ~M2, ~M3, ~M4, ~M5, ~M6, ~M7, ~M8,
        "Num. Obs.", diag1$nobs, diag2$nobs, diag3$nobs, diag4$nobs, diag5$nobs, diag6$nobs, diag7$nobs, diag8$nobs,
        "Sargan p", diag1$sargan, diag2$sargan, diag3$sargan, diag4$sargan, diag5$sargan, diag6$sargan, diag7$sargan, diag8$sargan,
        "AR(1) p", diag1$ar1, diag2$ar1, diag3$ar1, diag4$ar1, diag5$ar1, diag6$ar1, diag7$ar1, diag8$ar1,
        "AR(2) p", diag1$ar2, diag2$ar2, diag3$ar2, diag4$ar2, diag5$ar2, diag6$ar2, diag7$ar2, diag8$ar2
    )
)
...
kableExtra::kbl(
    ...
    col.names = c("", "Lines Added\n(Complexity)", "Lines Added\n(Static Analysis Warnings)",
                  "Cognitive\nComplexity\n(Lines)", "Static Analysis Warnings\n(Lines)",
                  "Commits\n(Complexity)", "Commits\n(Static Analysis Warnings)",
                  "Cognitive\nComplexity\n(Commits)", "Static Analysis Warnings\n(Commits)"),
    align = c("l", "c", "c", "c", "c", "c", "c", "c", "c"),
    caption = "Dynamic Panel GMM Estimates: Effects of Code Quality on Productivity (Lines Added vs Commits)",
    escape = FALSE
) %>%
    ...
    kableExtra::footnote(
        general = c(
            ...
            "Models 1-2, 5-6: lagged quality effects on productivity. Models 3-4, 7-8: contemporaneous productivity → quality.",
            "Contemporaneous productivity instrumented with lags 2-3 to address endogeneity.",
