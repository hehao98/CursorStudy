...
dynamic_model_commits2 <- pgmm(
    formula = commits ~ lag(commits, 1) + lag(code_smells, 1) +
        post_event +
        log(ncloc + 1) + log(age + 1) + log(contributors + 1) + log(stars + 1) +
        log(issues + 1) |
        lag(commits, 2),
...
# Test: Commits_{i,t} → Complexity_{it}
# Use lagged commits as instrument for contemporaneous commits
...
```{r dynamic_model_code_smells_commits}
# Test: Commits_{i,t} → TechnicalDebt_{it}
# Use lagged commits as instrument for contemporaneous commits

dynamic_model_code_smells_commits <- pgmm(
    formula = code_smells ~ lag(code_smells, 1) +
        commits +  # Contemporaneous effect (potentially endogenous)
        post_event +
        log(ncloc + 1) + log(age + 1) + log(contributors + 1) + log(stars + 1) +
        log(issues + 1) |
        lag(code_smells, 2:3) + lag(commits, 2:3),
    data = pdyn,
    effect = "twoways",
    model = "twosteps",
    transformation = "d",
...
summary(dynamic_model_code_smells_commits, robust = TRUE)
```
...
models_list <- list(
    ...
    "Code Smells\n(Lines)" = dynamic_model_code_smells,
    "Commits\n(Complexity)" = dynamic_model_commits,
    "Commits\n(Code Smells)" = dynamic_model_commits2,
    "Cognitive\nComplexity\n(Commits)" = dynamic_model_complexity_commits,
    "Code Smells\n(Commits)" = dynamic_model_code_smells_commits
)
...
diag8 <- get_diagnostics(dynamic_model_code_smells_commits)
...
coef8 <- extract_coefs(dynamic_model_code_smells_commits)
