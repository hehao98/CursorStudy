...
panel_data <- panel_data[panel_data$event < 202401 | panel_data$event > 202407]
...
run_did_analysis <- function(outcome_var, data) {
    did_result <- att_gt(
        ...
        idname = "repo_name",
        tname = "time",
        xformla = ~ ncloc + age + contributors + stars + issues,
        data = data,
        panel = TRUE,
...
key_titles <- c("Commits", "Lines Added", "Cognitive Complexity", "Duplicated Lines Density")
