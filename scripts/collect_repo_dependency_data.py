...
def setup_csv_files() -> Tuple[Dict, set]:
    ...
    if not os.path.exists(DEPENDENCY_DECLARATIONS_CSV):
        with open(DEPENDENCY_DECLARATIONS_CSV, 'w') as f:
            f.write("repo_name,week,commit_sha,dependency,version,concreteVersion,dependency_type\n")
        project_names = set()
    else:
        ...
        if "concreteVersion" not in header:
            ...
            if dep_type_col is None:
                ...
                with open(DEPENDENCY_DECLARATIONS_CSV, 'w') as new_f:
                    new_f.write("repo_name,week,commit_sha,dependency,version,concreteVersion,dependency_type\n")
            else:
                ...
                with open(DEPENDENCY_DECLARATIONS_CSV, 'w') as new_f:
                    ...
                    for _, row in old_data.iterrows():
                        new_f.write(f"{row['repo_name']},{row['week']},{row['commit_sha']},{row['dependency']},{row['version']},{row['version']},{row[dep_type_col]}\n")
...
def resolve_semantic_version(package_name: str, semantic_version: str, week: str) -> str:
    """
    ...
    try:
        ...
        if response.status_code != 200:
            logging.warning("Failed to fetch npm info for %s: HTTP %s", package_name, response.status_code)
        ...
        for version in package_data["versions"].keys():
            if version in package_data["time"]:
                try:
                    ...
                    logging.debug("Error parsing date for %s@%s: %s", package_name, version, str(e))
                    continue
        ...
        if semantic_version.startswith("^"):
            ...
            try:
                ...
                for v in valid_versions:
                    try:
                        ...
                        if base_version.major >= 1:
                            ...
                            if (v_parsed.major == base_version.major and 
                                v_parsed >= base_version):
                                compatible_versions.append(v)
                        else:
                            # For versions < 1.0.0, only allow patch updates
                            if (v_parsed.major == 0 and 
                                v_parsed.minor == base_version.minor and 
                                v_parsed >= base_version):
                                compatible_versions.append(v)
                    except (ValueError, AttributeError):
                        continue
                ...
                if compatible_versions:
                    ...
                    return max(compatible_versions, key=lambda x: semver.VersionInfo.parse(x))
                return semantic_version
            except (ValueError, AttributeError):
                return semantic_version
        ...
        elif ">" in semantic_version or "<" in semantic_version or "-" in semantic_version:
        ...
        logging.error("Failed to resolve version for %s@%s: %s", package_name, semantic_version, str(e))
        return semantic_version
    ...
    with open(DEPENDENCY_DECLARATIONS_CSV, 'a') as declarations_file:
        ...
        for dep_dict_key, dep_type in dep_type_map.items():
            ...
            for dependency, version in dependencies.items():
                ...
                declarations_file.write(f"{repo_name},{week},{commit_hash},{dependency},{version},{concrete_version},{dep_type}\n")
...
def pull_package_json_file_from_repo(repo_path: Path, commit_hash: str) -> Optional[Dict]:
    """
